...
Another way to illustrate it is to run the [Dijkstra](dijkstra/dijkstra.md) algorithm for all the vertices in the graph.
...
Note: if you repeat $|V|$ time the [Dijkstra](dijkstra/dijkstra.md) algorithm instead, the complexity become $O(|V|^2 \log |V|+|V||E|)$. It's better, but it works only with positive weighted edges.
